...
export const signInHandler = async (req, res) => {
  ...
  try{
    ...
    const isMatch = await bcrypt.compare(password, checkUserInDatabase.password);
    if (!isMatch) return res.status(401).json({ message: 'Invalid credentials' });
    const token = jwt.sign({ id: checkUserInDatabase._id  }, process.env.JWT_SECRET, { expiresIn: '11h' });
...
res.cookie('token', token, { httpOnly: true });
    ...
    console.log(error)
    res.status(500).json({ message: 'Error signup in', error: err.message });
  }
}
